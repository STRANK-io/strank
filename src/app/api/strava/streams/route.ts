 ...
 * - Strava 평균파워 계산 안정화를 위해 watts/moving 강제 포함
 */
export async function GET(request: NextRequest) {
  try {
    ...
    // 기본 키 + moving 포함 (Strava 방식 평균파워에 필요)
    const DEFAULT_KEYS = [
      'distance',
      'time',
      'latlng',
    ...
    // 사용자 지정 keys를 받되, watts/moving은 반드시 포함
    const rawKeys = searchParams.get('keys')?.split(',').map(k => k.trim()).filter(Boolean) || DEFAULT_KEYS
    const REQUIRED_FOR_AVG = ['watts', 'moving']
    const keys = Array.from(new Set([...rawKeys, ...REQUIRED_FOR_AVG]))
    ...
    // 스트라바 스트림 데이터 요청
    // ⚠️ 유틸 함수(fetchStravaActivityStreams) 내에서 key_by_type=true가 붙는지 확인하세요.
