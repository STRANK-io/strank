...
export async function generateActivityDescriptionWithGPT(
  ...
  try {
    ...
    // 템플릿 준비
    const template = `🚴 STRANK AI 라이딩 리포트
📅 [년-월-일-요일]
...
🏆 Powered by STRANK`
    ...
    const prompt = `주이 템플릿은 스트라바의 서드파티로 디스크립션에 작성되는 내용이야.
...
추정 조건
- 최대파워(W) 수치는 5초~2분 구간의 추정 상한값 기준
 ...
 STRANK와 함께한 오늘, 굿라이드였습니다! 🚴`
    ...
    try {
      ...
      const apiCall = openai.chat.completions.create({
        model: 'gpt-3.5-turbo',  // 공식 GPT-4 모델 사용
        messages: [
          {
            role: 'system',
            content:
              '당신은 사이클링 전문 코치이자 퍼포먼스 데이터 분석 전문가입니다. 사용자의 활동 데이터를 기반으로 정밀한 피트니스 인사이트를 제공하고, 준프로급 라이더를 목표로 효율적인 훈련 전략과 개선 방향을 제시합니다. 모든 분석은 실전 주행 데이터를 바탕으로 과학적이며 실용적으로 조금은 위트있게 전달되어야 합니다.',
          },
          {
            role: 'user',
            content: prompt,
