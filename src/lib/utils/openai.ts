...
■ 훈련분석
스트림 데이터 분석 결과를 기반으로 지속성, 강도 변화, 리듬 측면의 주요 특징을 분석하고, 개선이 필요한 다음 훈련 요소를 예시와 같이 가독성이 높게 이모지도 포함해서 100자 이내로 제안해줘.
예시)
중강도 지속 주행
꾸준한 페달링과 높은 심박 체류로 유산소 한계 지점 도달

■ 회복 가이드
스트림 데이터 분석 결과를 기반으로 회복 필요 수준을 평가하고, 회복 시간과 회복 방식(예: 휴식, 가벼운 회복라이딩, 스트레칭 등)을 예시와 같이 가독성이 높게 이모지도 포함해서 80자 이내로 제안해줘.
예시)
?~??시간, 충분한 휴식 추천 (심박 회복 우선)

■ 다음 훈련 추천
스트림 데이터 분석 결과를 바탕으로 다음 훈련에서 진행할 구체적인 훈련법(시간, 세트 수, 목적 포함)을 예시와 같이 매번 반복적이지 않게 아래 참고내용을 참고해서 120자 이내로 제안해줘.
예시)
...
■ 회복 식단
스트림 데이터 분석 결과를 기반으로,
적합한 식단을 탄수화물·단백질·수분으로 나눠서 자전거 라이더에게 적합한 식단으로 반복되지 않게 다양한 식단으로 구성해줘. (주로 바나나가 표현되고 있으니 자제해줘) 식단에 맞춘 취득칼로리를 알려줘.
예시)
