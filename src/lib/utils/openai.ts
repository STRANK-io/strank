...
■ 중장기 목표
스트림 데이터 분석 결과를 바탕으로 향후 1~2개월 내 달성 가능한 훈련 목표를 예시와 같이 템플릿 변경없이 150자 이내로 제안해줘.(예: FTP 증가, 언덕 지속 능력 강화, 장거리 페이스 유지 등)
[ftp60]의 값이 0이거나 N/A나 None 값이면 값을 추측하지말고 데이터 부족으로 표시해줘.예시의 폼은 유지해줘.
예시)
⏱ 현재 파워 프로필: [ftp20]W(20분 평균) → FTP 추정 [ftp60]W 
⏱ 목표 FTP: ???W (약 +?% 향상)
🏁 ???????????????????????????????
