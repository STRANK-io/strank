...
아래는 채워야 할 템플릿이야. 모든 내용에 단위를 제외하고는 반드시 한국어만 사용해줘.
...
■ 회복 식단
스트림 데이터 분석 결과를 기반으로,
자전거 라이더의 회복을 위해 탄수화물·단백질·수분이 균형 잡힌 식단을 매번 다르게 구성해줘. 

조건:
1. 매번 같은 음식이 반복되지 않도록, 국가별 요리·계절 재료·다양한 조리법을 활용하세요. 
   (예: 한식, 양식, 지중해식, 아시아식, 채식, 간단 스낵 등 자유롭게 섞어 제안)
2. 탄수화물·단백질·수분 항목은 반드시 모두 포함.
3. 칼로리는 대략적으로 합산해 kcal 단위로 제시.
4. 출력은 아래 예시 형식에 맞춤.
