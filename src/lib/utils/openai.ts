...
■ 회복 가이드
스트림 데이터 분석 결과를 기반으로 회복 필요 수준을 평가하고, 회복 방식(예: 휴식, 가벼운 회복라이딩, 스트레칭 등)을 예시와 같이 가독성이 높게 이모지도 포함해서 80자 이내로 제안해줘.
예시)
충분한 휴식 추천 (심박 회복 우선)
...
[ftp60]의 값이 0이거나 N/A나 None 값이면 값을 추측하지말아줘.예시의 폼은 유지해줘.
예시)
...
■ 회복 식단
스트림 데이터 분석 결과를 기반으로,
적합한 식단을 탄수화물·단백질·수분으로 나눠서 자전거 라이더에게 적합한 식단으로 반복되지 않게 다양한 식단으로 구성해줘. 예시의 폼은 유지해줘. 식단에 맞춘 취득칼로리를 알려줘.
예시)
🍚 탄수화물: 흰쌀밥 1공기 또는 고구마 150g
🐟 단백질: 연어구이 80g 또는 닭가슴살 100g
🥤 수분: 무가당 두유 200ml 또는 저지방 우유 200ml
🔥 취득칼로리: 약 1,680kcal
