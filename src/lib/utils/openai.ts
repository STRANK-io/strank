...
모든 내용에 존표시는 (P)Z1, (H)Z1 이런 식으로 표현해서 파워존과 심박존이 섞여서 해석되지 않게 해줘.
...
■ 중장기 목표
스트림 데이터 분석 결과를 바탕으로 향후 1~2개월 이내로 달성 가능한 훈련 목표를 예시와 같이 템플릿 변경없이 150자 이내로 제안해줘. (예: FTP 증가, 언덕 지속 능력 강화, 장거리 페이스 유지 등)
