...
주의사항: 템플릿을 반드시 지키고 데이터 변경이 없도록 해줘. 만약, 라이딩 분석 데이터가 없거나 0이면, 해당 데이터를 추측하고 추측한 데이터 끝에(추측)이라고 표시해줘. (추측)표시는 라이딩 분석 정보에만 적용해줘, 랭킹정보에는 적용되면 안돼. 랭킹 데이터가 없는 경우는 (비공개상태)라고 표시해줘.
활동 데이터:
...
심박 Zone은 대부분 Zone 3~4에 비중을 더 주는게 좋겠어 (아래 수치기준을 참고해줘)
...
■ 회복 식단
이번 라이딩의 회복에 적합한 식단을 매번 반복되지 않는 다양한 식단으로 구성해줘.
예시)
