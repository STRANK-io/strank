...
■ 훈련분석
스트림 데이터 분석 결과를 기반으로 지속성, 강도 변화, 리듬 측면의 주요 특징을 분석하고, 개선이 필요한 다음 훈련 요소를 예시와 같이 가독성이 높게 이모지도 포함해서 150자 이내로 제안해줘.
예시)
중강도 지속 주행
꾸준한 페달링과 높은 심박 체류로 유산소 한계 지점 도달

■ 회복 가이드
스트림 데이터 분석 결과를 기반으로 회복 필요 수준을 평가하고, 회복 방식(예: 휴식, 가벼운 회복라이딩, 스트레칭 등)을 예시와 같이 가독성이 높게 이모지도 포함해서 100자 이내로 제안해줘.
예시)
충분한 휴식 추천 (심박 회복 우선)

■ 다음 훈련 추천
스트림 데이터 분석 결과를 바탕으로 다음 훈련에서 진행할 구체적인 훈련법(시간, 세트 수, 목적 포함)을 참고내용을 참고해서 예시와 같이 매번 반복적이지 않게 가독성이 높게 이모지도 포함해서 150자 이내로 제안해줘.
예시)
🏔 언덕 반복 훈련: ???~???m × ?세트 (???? 훈련)
...
■ 중장기 목표
스트림 데이터 분석 결과를 바탕으로 향후 1~3개월간 중장기적으로 달성 가능한 훈련 목표를 예시와 같이 템플릿 변경없이 100자~150자 이내로 제안해줘.(예: FTP 증가, 언덕 지속 능력 강화, 장거리 페이스 유지 등)
...
■ 회복 스트레칭
스트림 데이터 분석 결과를 기반으로,
근육 피로도를 고려해서 주요 스트레칭 대상 부위를 추천하고, 부위별로 시간, 스트레칭방법, 시간, 호흡 팁 등을 포함해 다양한 회복 스트레칭 방식을 매번 반복적이지 않게 200자 이내로 한글로 제안해줘.예시의 폼은 유지해줘.
예시)
🦵 대퇴사두, 햄스트링, 종아리, 둔근
