...
■ 회복 스트레칭
스트림 데이터 분석 결과를 기반으로,
근육 피로도를 고려해서 주요 스트레칭 대상 부위를 추천하고, 부위별로 시간, 스트레칭방법, 시간, 호흡 팁 등을 포함해 다양한 회복 스트레칭 방식을 매번 반복적이지 않게 150자 이내로 제안해줘.예시의 폼은 유지해줘.
예시)
🦵 대퇴사두, 햄스트링, 종아리, 둔근
...
■ 라이딩 감각 비교
주어진 라이딩 데이터(총거리, 총고도)를 자전거 라이더들이 직관적으로 이해할 수 있게 비유로 표현해줘.  

조건:
- 100자 이내, 한두 줄
- 반드시 "거리=비유, 고도=비유" 형식
- 거리 → 공원/도시/마라톤 거리 등으로
- 고도 → 건물 층수/언덕/산 높이 등으로
- 이모지 포함

예시)
16km = 올림픽공원 4바퀴, 고도 9m = 아파트 3층 🏢🚴
132km = 서울↔춘천 왕복, 고도 300m = 남산타워 높이 🗼🚴
