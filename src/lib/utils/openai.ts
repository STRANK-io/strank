...
🏆 Powered by STRANK`
    ...
    messages.push({
          role: 'user',
          content: `
모든 분석은 속도가 최우선이야 무조건 최대 빠르게 분석해야해!
...
주의사항: 템플릿을 반드시 지키고 데이터 변경이 없도록 해줘. 만약, 라이딩 분석 데이터가 없거나 0이면, 해당 데이터를 추측하고 추측한 데이터 끝에(추측)이라고 표시해줘. (추측)표시는 라이딩 분석 정보에만 적용해줘, 랭킹정보에는 적용되면 안돼. 소수점은 제외해줘.
활동 데이터:
...
라이딩 데이터를 바탕으로 이번 주행의 페이스 조절, 리듬등 관점에서 예시와 같이 요약해줘.
[참고내용]
🚲 초보형 (입문형 라이더)
🔥 스프린터 (단거리가속형)
⛰️ 클라이머 (산악언덕형)
...
■ 훈련분석
라이딩 데이터를 기반으로 지속성, 강도 변화, 리듬 측면의 주요 특징을 분석하고, 개선이 필요한 다음 훈련 요소를 예시와 같이 가독성이 높게 이모지도 포함해서 100자 이내로 제안해줘.
예시)
중강도 지속 주행
꾸준한 페달링과 높은 심박 체류로 유산소 한계 지점 도달

■ 회복 가이드
라이딩 데이터를 기반으로 회복 필요 수준을 평가하고, 회복 시간과 회복 방식(예: 휴식, 가벼운 회복라이딩, 스트레칭 등)을 예시와 같이 가독성이 높게 이모지도 포함해서 80자 이내로 제안해줘.
예시)
?~??시간, 충분한 휴식 추천 (심박 회복 우선)

■ 다음 훈련 추천
라이딩 기록을 바탕으로 다음 훈련에서 진행할 구체적인 훈련법(시간, 세트 수, 목적 포함)을 예시와 같이 매번 반복적이지 않게 아래 참고내용을 참고해서 120자 이내로 제안해줘.
예시)
...
■ 중장기 목표
라이딩 데이터를 바탕으로 향후 1~3개월간 중장기적으로 달성 가능한 훈련 목표를 예시와 같은 느낌으로 예시 템플릿 변경없이 80자~120자 이내로 제안해줘.(예: FTP 증가, 언덕 지속 능력 강화, 장거리 페이스 유지 등)
예시)
...
■ 회복 식단
라이딩 데이터를 기반으로,
적합한 식단을 탄수화물·단백질·수분으로 나눠서 자전거 라이더에게 적합한 식단으로 반복되지 않게 다양한 식단으로 구성해줘. (주로 바나나가 표현되고 있으니 자제해줘)
예시)
